...
path — admission (refl and grounded), refutation, definitional merging,
verbatim-repeat skipping, and unparseable output. No credentials, CI-able.
...
#eval show MetaM Unit from do
  ...
  -- skipped verbatim; max-comm admitted. Total: 3 facts.
  unless corpus.facts.size == 3 do
    throwError "expected 3 admitted facts, got {corpus.facts.size}"
  let pretty ← corpus.facts.mapM fun f =>
    return toString (← Meta.ppExpr f.stmt)
  unless pretty.toList ==
      ["∀ (a b : Nat), min a b = min b a",
       "∀ (n : Nat), n + 0 = n",
       "∀ (a b : Nat), max a b = max b a"] do
